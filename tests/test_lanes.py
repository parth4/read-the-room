...
def test_heat_classifier_defaults() -> None:
    ...
    assert cfg.overlap_rising == pytest.approx(0.48)
    assert cfg.overlap_hot == pytest.approx(0.86)
    assert cfg.density_cv_min == pytest.approx(0.10)
    assert cfg.rise_dwell_seconds == pytest.approx(0.70)
    assert cfg.hot_dwell_seconds == pytest.approx(0.40)
    assert cfg.meter_stride == 6
    assert cfg.lane_ema == pytest.approx(0.08)
    assert cfg.wave_ema == pytest.approx(0.12)
