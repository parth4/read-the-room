...
def test_manifest_lists_fixture_categories() -> None:
    ...
    assert {"calm", "rising", "hot", "silence_or_music", "laughter_applause"} <= cats
...
def test_music_steady_is_not_hot() -> None:
    audio = render_synth("music_steady", sr=CFG.sample_rate)
    score = classify_audio(audio, CFG.sample_rate, CFG)
    assert score.predicted is not HeatLevel.HOT
    assert score.max_rms < CFG.hot_rms
...
def test_fixture_scorecard_and_cli(tmp_path: Path) -> None:
    ...
    scores = score_clips(specs, CFG)
    report = format_report(scores, CFG)
...
def test_module_calibrate_entrypoint(tmp_path: Path) -> None:
    ...
    assert accuracy(score_clips(load_manifest(man), CFG)) == 1.0
